...
function contributionDetail(item) {
  ...
  if (item.paid > 0) return `${formatMoney(item.paid)} € versés depuis le début de la cotisation`;
...
function updateSchedulePreview() {
  ...
  if (!preview) return;
  const earliestStart = fund ? ["2021-01", String(fund.start_date || "2021-01").slice(0, 7)].sort().at(-1) : "2021-01";
  const valid = Boolean(member && fund && start && end && start >= earliestStart && start <= end && end <= scheduleLimitMonthValue());
  if (settleButton) {
  ...
  const difference = monthDifference(end, current);
  const paidMonths = monthDifference(start, end) + 1;
  const inferredPaid = paidMonths * Number(fund.monthly_amount);
  if (difference > 0) {
    preview.innerHTML = `<span>${escapeHTML(member.full_name)} • ${escapeHTML(fund.name)}</span><strong>${formatMoney(inferredPaid)} € versés depuis ${formatPeriod(start)}</strong><small>${paidMonths} mensualité${paidMonths > 1 ? "s" : ""} considérée${paidMonths > 1 ? "s" : ""} réglée${paidMonths > 1 ? "s" : ""} • ${difference} mensualité${difference > 1 ? "s" : ""} due${difference > 1 ? "s" : ""} depuis ${formatPeriod(nextMonthValue(end))}</small>`;
  } else if (difference < 0) {
    const advance = Math.abs(difference);
    preview.innerHTML = `<span>${escapeHTML(member.full_name)} • ${escapeHTML(fund.name)}</span><strong>${formatMoney(inferredPaid)} € versés depuis ${formatPeriod(start)}</strong><small>${paidMonths} mensualité${paidMonths > 1 ? "s" : ""} réglée${paidMonths > 1 ? "s" : ""} • ${advance} mois d’avance jusqu’à ${formatPeriod(end)}</small>`;
  } else {
    preview.innerHTML = `<span>${escapeHTML(member.full_name)} • ${escapeHTML(fund.name)}</span><strong>${formatMoney(inferredPaid)} € versés depuis ${formatPeriod(start)}</strong><small>${paidMonths} mensualité${paidMonths > 1 ? "s" : ""} réglée${paidMonths > 1 ? "s" : ""} • à jour aujourd’hui</small>`;
  }
}
...
function hydrateScheduleForm() {
  ...
  document.querySelector("#schedule-start").min = ["2021-01", String(fund.start_date || "2021-01").slice(0, 7)].sort().at(-1);
  document.querySelector("#schedule-end").min = document.querySelector("#schedule-start").min;
  document.querySelector("#schedule-start").value = existing ? String(existing.start_month).slice(0, 7) : defaultStart;
  document.querySelector("#schedule-end").value = existing?.paid_through_month ? String(existing.paid_through_month).slice(0, 7) : currentMonthValue();
...
function validScheduleValues({ memberId, member, fund, start, end }) {
  const earliestStart = fund ? ["2021-01", String(fund.start_date || "2021-01").slice(0, 7)].sort().at(-1) : "2021-01";
  return Boolean(memberId && member && fund && canWriteFund(fund.code) && start && end && start >= earliestStart && start <= end && end <= scheduleLimitMonthValue());
}
